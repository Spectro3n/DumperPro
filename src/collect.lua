...
-- ══════════════════════════════════
--  NIL INSTANCES (shallow — 2 levels, NO recursion)
-- ══════════════════════════════════
...
local function scanInstances()
    ...
    C.safeScan("Instances", function()
        ...
        for i = 1, sz do
            ...
            pcall(function()
                local obj = all[i]
                if C.isScript(obj) then C.enqueue(obj, "memory") end
                C.checkRemote(obj)
            end)
            C.tick()
        end
...
local function collectionServiceScan()
    ...
    C.safeScan("Tags", function()
        ...
        for ti, tag in ipairs(tags) do
            if D.S.cancel or ti > 300 then break end
            pcall(function()
...
local function moduleRequireTrace()
    ...
    C.safeScan("RequireTrace", function()
        ...
        for mi, mod in ipairs(mods) do
            if D.S.cancel or mi > 400 then break end
            pcall(function()
                local fn
...
local function deepUpvalueChain()
    ...
    local function walkFunc(fn, depth)
        if depth > 5 or walked > maxWalk or D.S.cancel then return end
            ...
            for _, v in pairs(ups) do
                ...
                elseif type(v) == "table" then
                    ...
                    for _, tv in pairs(v) do
                        tc = tc + 1; if tc > 10 then break end
        ...
        if walked % 80 == 0 then C.tick() end
    end
...
function M.collectAll()
    ...
    D.UI:Log("Collecting ("..mode:upper()..")...", "blue")
    task.wait(0.3)
    ...
    if not D.limits.skipAdvanced and not D.S.cancel then
        ...
        if mode == "turbo" and not D.S.cancel then
            task.wait(0.2); C.memoryGuard()
            deepUpvalueChain()
        end
    end
    ...
    local srcStr = ""
    for m, c in pairs(D.S.stats.sources) do srcStr = srcStr..m..":"..c.." " end
