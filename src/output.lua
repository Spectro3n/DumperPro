            ...
            for _, r in ipairs(cat.items) do
                ...
                if #r.callbacks > 0 then
                    for _, cb in ipairs(r.callbacks) do
                        ...
                        if cb.sourceScript then L[#L+1] = "-- │   Script: " .. cb.sourceScript end
                        if cb.source then
                            local lc = 0
                            for line in cb.source:gmatch("[^\n]+") do
                                lc = lc+1
                                if lc > 20 then L[#L+1] = "-- │   ..."; break end
                                L[#L+1] = "-- │   " .. line
                            end
                        end
                    end
...
local function saveConnections()
    ...
    for _, c in ipairs(D.S.connections) do
        if not bySignal[c.signal] then
            bySignal[c.signal] = {}
            order[#order+1] = c.signal
        end
    ...
    for _, sig in ipairs(order) do
        ...
        for _, c in ipairs(conns) do
            ...
            L[#L+1] = "  [" .. c.funcType .. "] " .. disp
                .. (c.enabled and "" or " (OFF)")
        end
        L[#L+1] = ""
    end
        ...
        local function walk(obj, depth)
            ...
            pcall(function()
                if obj:IsA("GuiObject") then
                    info = info .. " V=" .. tostring(obj.Visible)
                end
            end)
            pcall(function()
...
local function saveGameInfo()
    ...
    for m, c in pairs(s.methods) do
        L[#L+1] = string.format("  %-20s %d", m, c)
    end
    L[#L+1] = ""

    L[#L+1] = "── DISCOVERY SOURCES ──"
    for m, c in pairs(s.sources) do
        L[#L+1] = string.format("  %-25s %d", m, c)
    end
    L[#L+1] = ""

    L[#L+1] = "── MODE LIMITS ──"
    for k, v in pairs(D.limits) do
        L[#L+1] = string.format("  %-22s %s", k, tostring(v))
    end
